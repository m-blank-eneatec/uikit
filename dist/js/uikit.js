...
(function (global, factory) {
    ...
    function addClass(element, ...classes) {
      for (const node of toNodes(element)) {
        const add = toClasses(classes).filter((cls) => !hasClass$1(node, cls));
        if (add.length) {
    ...
    function removeClass(element, ...classes) {
      for (const node of toNodes(element)) {
        const remove = toClasses(classes).filter((cls) => hasClass$1(node, cls));
        if (remove.length) {
    ...
    const Transition = {
      ...
      inProgress(element) {
        return hasClass$1(element, clsTransition);
      }
    };
    ...
    function getCoveringElement(target) {
      ...
      return target.ownerDocument.elementsFromPoint(left + width / 2, top).find(
        ...
        !hasClass$1(el, "uk-togglable-leave") && (hasPosition(el, "fixed") && zIndex(
    ...
    function fade(action, target, duration, stagger = 0) {
      ...
      return hasClass$1(target, clsLeave) ? waitTransitionend(target).then(enterFn) : hasClass$1(target, clsEnter) ? waitTransitionend(target).then(leaveFn).then(enterFn) : leaveFn().then(enterFn);
    }
    ...
    var filter = {
      ...
      methods: {
        ...
        getState() {
          return this.toggles.filter((item) => hasClass$1(item, this.cls)).reduce((state, el) => mergeState(el, this.attrItem, state), {
            filter: { "": "" },
            sort: []
          });
        },
    ...
    var Togglable = {
      ...
      methods: {
        ...
        isToggled(el = this.$el) {
          el = toNode(el);
          return hasClass$1(el, this.clsEnter) ? true : hasClass$1(el, this.clsLeave) ? false : this.cls ? hasClass$1(el, this.cls.split(" ")[0]) : isVisible(el);
        },
        _toggle(el, toggled) {
          if (!el) {
            return;
          ...
          let changed;
          if (this.cls) {
            changed = includes(this.cls, " ") || toggled !== hasClass$1(el, this.cls);
    ...
    var SliderDrag = {
      ...
      methods: {
        ...
        move(e) {
          ...
          let width = getDistance$1.call(this, prevIndex, nextIndex);
          while (nextIndex !== prevIndex && dis > width) {
            this.drag -= width * this.dir;
            prevIndex = nextIndex;
            dis -= width;
            nextIndex = this.getIndex(prevIndex + this.dir);
            width = getDistance$1.call(this, prevIndex, nextIndex);
          }
    ...
    function getDistance$1(prev, next) {
    ...
    var Slider = {
      ...
      methods: {
        show(index, force = false) {
          ...
          const prev = hasClass$1(this.slides, this.clsActive) && this.slides[prevIndex];
        ...
        },
        {
          ...
          async handler(_, item) {
            ...
            if (type === "image" || src.match(/\.(avif|jpe?g|jfif|a?png|gif|svg|webp)($|\?)/i)) {
              const img = createEl$1("img", { src, alt, ...attrs });
              on(img, "load", () => this.setItem(item, img));
              on(img, "error", () => this.setError(item));
            } else if (type === "video" || src.match(/\.(mp4|webm|ogv)($|\?)/i)) {
              const video = createEl$1("video", {
                src,
                poster,
                controls: "",
                playsinline: "",
    ...
    var lightbox = {
      ...
      methods: {
        show(index) {
          const items = uniqueBy(this.toggles.map(toItem$1), "source");
          if (isElement(index)) {
            const { source } = toItem$1(index);
    ...
    function install$4(UIkit, Lightbox) {
      if (!UIkit.lightboxPanel) {
        UIkit.component("lightboxPanel", LightboxPanel);
      }
      assign(Lightbox.props, UIkit.component("lightboxPanel").options.props);
    }
    function toItem$1(el) {
      const item = {};
    ...
    ***************************************************************************** */
    ...
    var lightboxPlus = {
      ...
      data: {
        toggle: "a",
        swipeThreshold: 10
        // same threshold as in src/js/mixin/slider-drag.js
      },
      computed: {
        toggles: ({ toggle }, $el) => $$(toggle, $el)
      },
    ...
    function install$3(UIkit, LightboxPlus) {
    ...
    function toItem(el) {
      const item = {};
      for (const attr2 of ["srcset", "source", "href", "caption", "type", "poster", "alt", "attrs"]) {
        item[attr2 === "href" ? "source" : attr2] = data(el, attr2);
      }
      item.attrs = parseOptions(item.attrs);
      return item;
    ...
    function useTriggers(cmp) {
      ...
      function triggerShow(el) {
        if (hasClass$1(el, clsLeave)) {
          triggerHide(el);
          triggerHidden(el);
        }
        if (!hasClass$1(el, clsSlideActive)) {
          trigger(el, "beforeitemshow", [cmp]);
          trigger(el, "itemshow", [cmp]);
        }
      }
      function triggerShown(el) {
        if (hasClass$1(el, clsEnter)) {
    ...
    var components$1 = /*#__PURE__*/Object.freeze({
        ...
        LightboxPlus: lightboxPlus,
        LightboxPlusPanel: LightboxPlusPanel,
    ...
    var Accordion = {
      ...
      watch: {
        items(items, prev) {
          if (prev || hasClass$1(items, this.clsOpen)) {
            return;
          }
    ...
    var drop = {
      ...
      update: {
        write() {
          if (this.isToggled() && !hasClass$1(this.$el, this.clsEnter)) {
            this.position();
          }
        }
      },
    ...
    var Dropnav = {
      ...
      methods: {
        ...
        isDropbarDrop(el) {
          return includes(this.dropdowns, el) && hasClass$1(el, this.clsDrop);
        },
    ...
    function getGutter(rows, cls) {
      const node = rows.flat().find((el) => hasClass$1(el, cls));
    ...
    const Search = {
      ...
      beforeConnect() {
        const isToggle = hasClass$1(this.$el, "uk-search-toggle") || hasClass$1(this.$el, "uk-navbar-toggle");
        this.icon = isToggle ? "search-toggle-icon" : hasClass$1(this.$el, "uk-search-icon") && closest(this.$el, ".uk-search-large") ? "search-large" : closest(this.$el, ".uk-search-navbar") ? "search-navbar" : this.$props.icon;
    ...
    const Slidenav = {
      ...
      beforeConnect() {
        ...
        const icon = this.$props.icon;
        this.icon = hasClass$1(this.$el, "uk-slidenav-large") ? `${icon}-large` : icon;
      }
    };
    ...
    const Close = {
      ...
      i18n: { label: "Close" },
      beforeConnect() {
        this.icon = `close-${hasClass$1(this.$el, "uk-close-large") ? "large" : "icon"}`;
      }
    };
    const Marker = {
      extends: ButtonComponent,
    ...
    var modal = {
      ...
      events: [
        ...
        },
        {
          ...
          self: true,
          handler() {
            if (hasClass$1(this.panel, "uk-margin-auto-vertical")) {
    ...
    var navbar = {
      ...
      watch: {
        items() {
          const justify = hasClass$1(this.$el, "uk-navbar-justify");
      ...
      events: [
        {
          ...
          handler({ target }) {
            if (this.getTransparentMode(target) === "remove" && hasClass$1(this.navbarContainer, clsNavbarTransparent)) {
      ...
      methods: {
        getTransparentMode(el) {
          ...
          if (drop && hasClass$1(el, "uk-dropbar")) {
    ...
    var offcanvas = {
      ...
      events: [
        ...
        },
        {
          ...
          self: true,
          handler() {
            if (this.mode === "reveal" && !hasClass$1(parent(this.panel), this.clsMode)) {
      ...
      update: [
        {
          ...
          write({ active }) {
            const changed = active !== false && !hasClass$1(this.elements[active], this.cls);
          ...
          write(data, types) {
            ...
            if (this.inactive || scroll2 < start || this.showOnUp && (scroll2 <= start || dir === "down" && isScrollUpdate || dir === "up" && !this.isFixed && !below)) {
              ...
              if (this.animation && below) {
                if (hasClass$1(this.$el, "uk-animation-leave")) {
                  return;
                }
    ...
    var Switcher = {
      ...
      methods: {
        index() {
          return findIndex(this.children, (el) => hasClass$1(el, this.cls));
        },
        show(item) {
          const toggles = this.toggles.filter((el) => !matches(el, selDisabled));
          ...
          this.connects.forEach(async ({ children: children2 }) => {
            const actives = toArray(children2).filter(
              (child, i) => i !== active && hasClass$1(child, this.cls)
            );
    ...
    var tab = {
      ...
      connected() {
        const cls = hasClass$1(this.$el, "uk-tab-left") ? "uk-tab-left" : hasClass$1(this.$el, "uk-tab-right") ? "uk-tab-right" : false;
        if (cls) {
    ...
    var toggle = {
      ...
      methods: {
        async toggle(type) {
          ...
          const leaving = this.target.filter((el) => hasClass$1(el, this.clsLeave));
          if (leaving.length) {
